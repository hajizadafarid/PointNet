...
def step(points, pc_labels, class_labels, model):
    ...
    logits, transform_1, transform_2, feature_pointcloud = model(points)
    
    logits = logits.transpose(1, 2)
    loss_fn = torch.nn.CrossEntropyLoss()

    loss = loss_fn(logits, pc_labels)
    if type(transform_1) == torch.Tensor:    
        loss+=get_orthogonal_loss(transform_1)
    if type(transform_2) == torch.Tensor:
        loss+=get_orthogonal_loss(transform_2)

    preds = torch.argmax(logits, dim = 1)
    loss += loss_fn(preds.float(), class_labels)
